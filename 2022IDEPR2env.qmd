...
my_dat57 <- my_dat %>% filter(trial_name == "202268IDEPR_cere") %>% 
...
```{r}
my_dat57 %>% filter(rep_number == 1, row_number %in% c(11), 
                    col_number < 28, thrips1_5 >= 3) %>% 
  select(plot_number, row_number, col_number, plot_name, thrips1_5) %>% arrange(plot_number) %>% 
  write.table("clipboard", sep = "\t", col.names = T, row.names = F)
```
...
```{r}
# Yield
yield_vcor <- covcor_heat(matrix = met_results$VCOV$yield_ha$CORR, legend =c(0.35, 0.8))

ggsave(paste0("images\\yield_vcor", trial_interest, Sys.Date(), ".png"),
       plot = yield_vcor, units = "in", dpi = 300, width = 6, height = 6)

# Dry matter

DM_vcor <- covcor_heat(matrix = met_results$VCOV$DM_gravity$CORR, legend =c(0.35, 0.8))

ggsave(paste0("images\\DM_vcor", trial_interest, Sys.Date(), ".png"),
       plot = DM_vcor, units = "in", dpi = 300, width = 6, height = 6)
```
